...
import { obtenerOficioPorId, finalizarAsignacion } from '@/features/modulo-correspondencia/oficio/services/oficioService';
import '@/features/modulo-correspondencia/oficio/styles/asignar-area.css';

export const AsignarAreaOficioPage = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                const data = await obtenerOficioPorId(id);
                setMemoData(data);
            } catch (error) {
                console.error("Error al recuperar el oficio:", error);
            } finally {
                setCargando(false);
            }
        };
...
const handleConfirmarFinalizar = async () => {
    ...
    try {
        ...
        alert("Oficio Asignado y Enviado con Éxito");
       navigate('/correspondencia/lista-oficios-revision');
    } catch (error) {
 ...
 const handleDescargar = async () => {
    try {
        // ✅ ID correcto según el componente
        const elemento = document.getElementById('oficio-pdf-content');
        
        if (!elemento) {
            alert("No se encontró el contenido del oficio.");
            return;
        }
