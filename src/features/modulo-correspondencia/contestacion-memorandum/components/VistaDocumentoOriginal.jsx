...
import { extractMemorandumDocumentoPath } from '@/shared/utils/documentoUtils';

export const VistaDocumentoOriginal = ({ memorandum, loading: loadingMemo }) => {
  const urlRelativa = extractMemorandumDocumentoPath(memorandum);
  const { blobUrl, loading: loadingPdf, error, downloadUrl } = useDocumentoPdf(urlRelativa);

  if (loadingMemo || loadingPdf) {
